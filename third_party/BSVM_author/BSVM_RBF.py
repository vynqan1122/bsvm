# -*- coding: utf-8 -*-
"""
Verbose-traceable BSVM with RBF Kernel - Manuscript Figure Generator (Zipped PDFs)
Now includes:
 • Uses RBF Kernel and adds '_rbf' suffix to saved filenames.
...
def plot_decision_boundary(model, X, y, filename_base, h=None, yh=None, attempt_coord=None, attempt_label=None, mis_idx=None):
    ...
    # Save the figure as PDF with the '_rbf' suffix
    filename = f"{filename_base}_{FIG_COUNT:02d}_rbf.pdf"
...
def main():
    ...
    try:
        ...
        # --- CHANGED: Updated kernel to 'rbf' ---
        parameters = dict(C=10, kernel='rbf', gamma='scale', class_weight=class_weights)
